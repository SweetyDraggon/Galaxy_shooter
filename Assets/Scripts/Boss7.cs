...
public class Boss7 : MonoBehaviour
{
	...
	public List<UbhNwayShot> lsShotList;
	...
	private void Move()
	{
		...
		base.transform.DOMove(new Vector3(UnityEngine.Random.Range(-2.5f, 2.5f), 2f), UnityEngine.Random.Range(3f, 6f), false).SetDelay(1f).OnComplete(delegate
		{
			this.Attack();
		});
	}
	...
	{
		Boss7._SetDie_c__Iterator0 _SetDie_c__Iterator = new Boss7._SetDie_c__Iterator0();
	...
	private void ShotControl(int index)
	{
		...
		{
			this.shotCtrl.StopShot();
		}
		this.shotCtrl = this.lsShotList[index];
		if (index == 1)
		{
			int num = UnityEngine.Random.Range(0, 3);
			this.shotCtrl._CenterAngle = (float)((num != 0) ? ((num != 1) ? 200 : 180) : 160);
		}
		if (this.shotCtrl != null)
		{
			this.shotCtrl.Shot();
